...
-   **Model 1** is fitted to **ARIMA(1,1,3)(0,1,1)[12]** including both
    ...
    -   For the non-seasonal component, the model is **ARMA(1,3)** with
        **differencing 1**.

    -   For the seasonal component, the model has an MA part order of 1
        and **differencing with one season which is 12**.

-   **Box-Cox Transformation**:

    -   The lambda value is 0.1132106 applied to stabilize the variance
...
-   Similarly error measures and training errors are provided which is
    helpful to evaluate and compare the models
...
# model1
sarima.for(X_t, n.ahead=12,
           p=1, d=1, q=3, P=0, D=1, Q=1, S=12,
           main="SARIMA(1,1,3)(0,1,1)[12]",
           cex.main=0.8)
# model2
sarima.for(X_t, n.ahead=12,
           p=0, d=0, q=1,
...
mtext("12-Month Switzerland Turnover Forecasting",
...
-   **Forecast Plot**:
    ...
    -   The forecast values (in red) follow the observed values closely,
        maintaining the trend and seasonality.

    -   Since both models aim to capture the same level of confidence,
        the model with the narrower confidence interval is preferred
        indicating more accurate and reliable predictions.

    -   The Model 1 confidence intervals (shaded area) are reasonably
        narrow compared to Model2 indicating the better prediction.

#### Model 2: ARIMA(0,0,1) with Non-Zero Mean

-   **Forecast Plot**:

    -   The forecast values (in red) show a more pronounced deviation
...
par(mfrow = c(2, 1))
plot(Freq[1:(n/2)], Per[1:(n/2)], type = 'h', lwd = 3, ylab = "Periodogram", xlab = "Frequency", main = "Periodogram of Residuals")
plot(Freq[1:(n/2)], sPer[1:(n/2)], type = 'h', lwd = 3, ylab = "Scaled Periodogram", xlab = "Frequency", main = "Scaled Periodogram of Residuals")
...
#plot
tsplot(X_t, col="black",  main="Symmetric MA Smoothing (k=2, 6)") #plot original data X_t
lines(ma_6, lwd=2, col="blue") #plot MA with k=6
lines(ma_2, lwd=2, col="red") #plot MA with k=2

# Legend
legend("topleft", 
       legend=c("Original Data", "k=6", "k=2"), 
       col=c("gray70", "blue", "red"), 
       lwd=c(1, 2, 2), bty="n", cex=0.8)
```

-   **For** $k=2$:
    -   The symmetric moving average filter with $k=2$ provides a less
        smooth trend as it captures more short-term fluctuations. It
        better captures the seasonal behavior of the time series because
        the smaller window size allows for more detailed tracking of the
        data variations.
-   **For** $k=6$:
...
tsplot(X_t, col="black", main="Kernel Smooting")

legend("topleft", 
       legend=c("bandwidth=0.1", "bandwidth=0.5", "bandwidth=1", "bandwidth=2"), col=c("yellow", "blue","green", "red"), 
       lwd=c(2, 2, 2, 2), bty="n", cex=0.8)
...
# Lowess
plot(X_t, main="Lowess smoothing")
...
legend("topleft", legend = c("spar = 0.01", "spar = 0.5", "spar = 0.9", "cv = TRUE"), 
...
par(mfrow = c(2,1))
plot(Freq[1:n/2], Per[1:n/2], type='h', lwd=3, ylab="Periodogram", xlab="Frequency", main = "Periodogram of X_t")
#abline(h=sigma2, col = 2)
plot(Freq[1:n/2], sPer[1:n/2], type='h', lwd=3, ylab="Scaled Periodogram", xlab="Frequency", main = "Scaled Periodogram of X_t")
